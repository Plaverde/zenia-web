...
async function seed() {
  ...
  const sql = `-- Phase 2 seed: Admin user and services
-- El fichero se escribe en UTF-8 y se envia a mysql.exe por redireccion (<),
-- no por 'type |': la tuberia de cmd recodifica a la pagina OEM (850/437) y
-- corrompe los acentos. SET NAMES fija el charset del lado del cliente.
SET NAMES utf8mb4;
...
`;
  ...
  // Write SQL to file
  const fs = require('fs');
  fs.writeFileSync('prisma/seed-phase2-final.sql', sql, 'utf8');
  console.log('SQL file written to prisma/seed-phase2-final.sql');

  // Execute via MySQL
  console.log('Executing SQL...');
  try {
    execSync(`cmd /c "C:\\xampp\\mysql\\bin\\mysql.exe --default-character-set=utf8mb4 -u root zenia_db < prisma\\seed-phase2-final.sql"`, {
      stdio: 'inherit'
    });
    ...
    const dbHash = execSync('cmd /c "C:\\xampp\\mysql\\bin\\mysql.exe --default-character-set=utf8mb4 -u root zenia_db -N < prisma\\verify.sql"').toString().trim();
